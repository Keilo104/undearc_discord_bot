...
class Underarc:
    ...
    people_identities_directory = "markov_chain_stuffs/people_identities"
    people_identities = {}

    extraction_channels = []
    extraction_channels_json = "markov_chain_stuffs/extraction_channels.json"

    signed_up = []
    signed_up_json = "markov_chain_stuffs/peep_list.json"

    answers_8ball = []
    answers_8ball_json = "8ball_stuffs/8ball_answers.json"

    admins = []
    admins_json = "admins.json"

    def load_extraction_channels(self):
        with open(self.extraction_channels_json, "r") as extraction_channels_json_file:
            self.extraction_channels = json.load(extraction_channels_json_file)["channels"]

    def load_signed_up(self):
        with open(self.signed_up_json, "r") as signed_up_json_file:
            self.signed_up = json.load(signed_up_json_file)["peeps"]

    def load_8ball_answers(self):
        with open(self.answers_8ball_json, "r") as answers_8ball_json_file:
            self.answers_8ball = json.load(answers_8ball_json_file)["answers"]

    def load_peoples_identities(self):
        for person in os.listdir(self.people_identities_directory):
            with open(f"{self.people_identities_directory}/{person}", "r", encoding="utf-8") as person_file:
                self.people_identities[f"{person.split('.')[0]}"] = person_file.read()

    def load_admins(self):
        with open(self.admins_json, "r") as admins_json_file:
...
def save_people_identities():
    for person, messages in underarc.people_identities.items():
        person_file = open(f"{underarc.people_identities_directory}/{person}.txt", "w", encoding="utf-8")
